import { Box,Flex,Image, Link,Button,Icon,Heading} from "@chakra-ui/react";
            ...
            <Flex  gap={'3rem'}  >
                <Link href="/">
                    <Heading fontWeight={'600'}  color={activeLink==='/'? '#2E3192' :'black'} fontSize={'16px'}>
                        HOME
                     <Box h="3px" display={activeLink==='/'? 'block' :'none'} fontSize={'28px'} mt={'1.5rem'} borderRadius={'12px'} w="100%" bg="linear-gradient(90deg, #2E3192 0%, #1C55E0 100%)"></Box>
                    </Heading>
                </Link>
                <Link href="/about">
                ...
                <Link href="/contact">
                    <Heading fontWeight={'600'}  color={activeLink==='/contact'? '#2E3192' :'black'} fontSize={'16px'}>
                            CONTACT US 
                           <Box h="3px" display={activeLink==='/contact'? 'block' :'none'} fontSize={'28px'} mt={'1.5rem'} borderRadius={'12px'} w="100%" bg="linear-gradient(90deg, #2E3192 0%, #1C55E0 100%)"></Box>
                    </Heading>
                </Link>
            </Flex>
      ...
      <Flex bg={'white'}  mx={'auto'} textColor={'white'} display={{lg:'none',md:'flex',sm:'flex',base:'flex'}} >
        ...
        <Box zIndex={'100'}  display={showModileMenu?'block':'none'} bg={'#0A0A0A'} position={'fixed'} h={'100vh'} w={'100%'}>
           ...
           <Flex mt={'4rem'} gap={'1rem'} flexDir={'column'}>
                <Link mx={'auto'} onClick={()=>setShowMobileMenu(false)} href="#Features">Home</Link>
                <Link mx={'auto'} onClick={()=>setShowMobileMenu(false)} href="#Works">About Us</Link>
                <Link mx={'auto'} onClick={()=>setShowMobileMenu(false)} href="#Pricing">Contact Us</Link>
                <Link mx={'auto'} onClick={()=>setShowMobileMenu(false)} href="#About">Contact Us</Link>
            </Flex>
