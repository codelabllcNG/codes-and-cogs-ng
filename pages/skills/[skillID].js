...
function ToolID(props) {
    ...
    const fetchData = async () => {
     ...
     return
      ...
      if (data.status !== "success") {
        setSearchingTalents(false);
        setResponse("Something went wrong, could not load Talents for this skill, please refresh page.");
        console.log("Something went wrong, could not load Talents, please refresh page.");
        return;
      }
  ...
  return (
    <div className="px-5 md:px-10">
      ...
      <div className='mt-10'>
        {searchingTalents ? <div className='flex justify-center text-lg font-semibold'><p>{response}</p></div> : (talentsFound.length < 1 ? <div className='flex justify-center text-lg font-semibold text-red-600'><p>{response}</p></div> : <div >
