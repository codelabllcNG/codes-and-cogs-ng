...
export async function getStaticProps(context) {
  ...
  const response = await fetch(
    `${NEXT_PUBLIC_DEV_API_BASE}/codesandcogs/dev/api/codesandcogs/v1/remotelocations`
  );
...
export async function getStaticPaths() {
  const response = await fetch(
    `${NEXT_PUBLIC_DEV_API_BASE}/codesandcogs/dev/api/codesandcogs/v1/server/api/codesandcogs/v1/remotelocations`
  );
  const data = await response.json();
